...
\begin{abstract}
\section{}
The Hilbert--P\'olya conjecture proposes that the imaginary parts of the nontrivial zeros of the Riemann $\zeta$ function are the eigenvalues of a self-adjoint operator. Random matrix theory (RMT) provides a powerful description of the \emph{local} spectral statistics of the zeros: after the standard unfolding procedure, their pair correlation matches that of the Gaussian Unitary Ensemble (GUE), with subleading arithmetic corrections governed by the Hardy--Littlewood conjectures. Building on our recent finding that the symbolic dynamics of the Logistic map at its band-merging point is isomorphic to the prime sieve [Wang, \emph{Research in Mathematics} 13(1), 2026], we ask a complementary, constructive question: can a single low-dimensional \emph{deterministic} dynamical system reproduce, without manual unfolding, both the global counting function of the zeros and their local level repulsion? The one-dimensional Logistic map is dissipative ($\det J \to 0$) and therefore cannot host a unitary, time-reversible spectrum; we lift it to the two-dimensional area-preserving Hénon map ($\det J = 1$), which restores the structure required by Hilbert--P\'olya. We then perform a spectral analysis of this map near the edge of chaos. Using a continuous Hamiltonian obtained from the map's continuum limit, regularized by a quartic confining term, we construct two independent numerical eigensolvers --- a unitary Fourier split-operator (quantum) solver and a Markovian Fokker--Planck (dissipative) solver --- and compare their spectra to the first 100 Riemann zeros. With a single anchoring point and no global fitting, the quantum solver reaches a best (finely tuned) mean absolute percentage error of $2.3\%$ on the first 100 zeros, and --- re-optimizing the resolution at each setting --- a robust $\approx 10$--$20\%$ agreement across grids and regularization strengths; the dissipative solver reaches $6.5\%$. A globally least-squares-fitted GUE surrogate shows systematic structural deviation in the same comparison. Computing the standard local spectral diagnostics, we find that the Floquet eigenphases of the quantum solver exhibit GUE-type level repulsion consistent with the Riemann zeros (in a limited-statistics regime); we emphasize that such GUE statistics are generic to chaotic time-reversal-breaking systems and serve as a consistency check, whereas the distinctive and far more constrained result is the agreement with the actual zero values. We also note a qualitative resemblance between the high-frequency breakdown of our solvers and decoherence profiles reported for superconducting quantum hardware. We present these results as numerical observations and heuristic arguments rather than proofs, and we summarize explicitly which statements are conjectural, numerical, or established.
