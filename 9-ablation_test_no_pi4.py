...
def holy_grail_objective_locked_nocutoff(x):
    hbar, a_start = x
    
    if hbar <= 0.01 or hbar >= 0.35 or a_start <= 1.03:
        return 1e6

    # 💣 致命剥除：L 拉宽至 6.0 彻底撤掉防坠网，N 提高到 400 维持高清！
    N = 400         
    L = 6.0         
    T_steps = 300   
    a_end = 1.02    # 【死锁 1.02 的无底深渊】
    c_offset = 10.0 
...
def callback_fn(xk, convergence):
    ...
    generation += 1
    current_mse = holy_grail_objective_locked_nocutoff(xk)
    if current_mse < best_mse_global:
        best_mse_global = current_mse
    elapsed = time.time() - global_start_time
    # convergence 会一直保持很低，享受绝望吧
    print(f" [💥 1号机 死锁 1.02 深渊熬鹰: 第 {generation} 代] 耗时: {elapsed/60:.2f}分钟 | 基因纯度: {convergence*100:.2f}%")
    print(f"    👉 当代绝望妥协参数: hbar={xk[0]:.6f}, a_start={xk[1]:.6f} | 绝望 MSE = {current_mse:.4f}\n")

def run_epyc_holy_grail_nocutoff():
    global global_start_time
    max_cores = multiprocessing.cpu_count()
    WORKERS = max(1, max_cores - 2)
    
    print("====================================================================")
    print(f" 🚀 AMD EPYC [1号母舰·深渊死锁] 点火！物理算力核心: {max_cores} ")
    print("    [实验代号]: 无底深渊的容量崩塌 (Abyss Capacity Collapse) ")
    print("    [引擎模式]: 移除 0.05*q**4，边界推至 L=6.0，死磕一整晚！ ")
    print("====================================================================\n")
    
    bounds = [(0.02, 0.35), (1.05, 1.85)]
    global_start_time = time.time()
    
    # 🔥 彻夜熬鹰·地狱火狂暴参数
    result_de = differential_evolution(
        holy_grail_objective_locked_nocutoff, bounds, 
        strategy='rand1bin',   # 💣 全局盲搜，拒绝抱团苟活
        maxiter=10000, 
        popsize=100,           # 💣 种群拉爆
        tol=0.0,               # 💣 相对收敛死线清零，永不停机
        atol=0.0,              # 💣 绝对收敛死线清零
        mutation=(0.5, 1.9),   # 💣 极其狂暴的变异率
        recombination=0.7, 
        workers=-1, updating='deferred', callback=callback_fn, 
        polish=False,          # 💣 关闭抛光，省点算力全用来撞南墙
        disp=False
    )
    
    print("\n" + "💣 1号机 崩溃验证结束 " + "="*50)
    print(f"【死锁 1.02 拓扑 (深渊裸奔) 挣扎极限】:")
    print(f"[*] 畸变平滑尺度 (hbar)          = {repr(result_de.x[0])}")
    print(f"[*] 畸变大爆炸起点 (a_start)     = {repr(result_de.x[1])}")
    print(f"[*] 无墙无盖的真实发散 MSE       = {repr(result_de.fun)}")
    print("="*64 + "\n")

if __name__ == '__main__':
    run_epyc_holy_grail_nocutoff()