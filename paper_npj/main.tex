%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% npj Complexity submission -- generic LaTeX (article) initial-submission format
% "An Area-Preserving Henon-Map Model for the Riemann Zeros"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% naturemag-doi.bst emits \JournalTitle (defined in wlscirep.cls); provide it here.
\providecommand{\JournalTitle}[1]{\textit{#1}}

\title{\bfseries An Area-Preserving H\'enon-Map Model for the Riemann Zeros:
A Deterministic-Dynamics Approach with Quantum and Dissipative Solvers}

\author[1]{Liang Wang\thanks{Correspondence: wangliang.f@gmail.com}}
\affil[1]{School of Artificial Intelligence and Automation, Huazhong University of Science and Technology, Wuhan, P.R. China}
...
\begin{abstract}
\noindent
The Hilbert--P\'olya conjecture proposes that the imaginary parts of the nontrivial Riemann zeros are eigenvalues of a self-adjoint operator, and random matrix theory reproduces their unfolded local statistics (the Gaussian Unitary Ensemble, GUE). Building on our result that the symbolic dynamics of the Logistic map at its band-merging point is isomorphic to the prime sieve [Wang, \emph{Research in Mathematics} 13(1), 2026], we ask whether a single low-dimensional deterministic system can reproduce both the global counting function of the zeros and their local repulsion. The one-dimensional Logistic map is dissipative ($\det J\to0$); we therefore lift it to the two-dimensional area-preserving H\'enon map ($\det J=1$), which admits a unitary spectrum, and study it near the edge of chaos. From the map's continuum limit we build a quartic-regularized Hamiltonian and two independent solvers --- a unitary Fourier (quantum) solver and a Markovian (dissipative) solver --- and compare to the first 100 zeros. Under single-point anchoring the quantum solver reaches a best mean absolute percentage error of $2.3\%$ (a sharp optimum) and a robust $\approx10$--$20\%$ across grids and regularizations; the dissipative solver reaches $6.5\%$. Its Floquet eigenphases show GUE-type level repulsion. We present these as numerical observations rather than proofs, stating explicitly which claims are conjectural, numerical, or established.
\end{abstract}

\noindent\textbf{Keywords:} Riemann zeros; H\'enon map; area-preserving dynamics; quantum chaos; Hilbert--P\'olya conjecture; Fokker--Planck dynamics; spectral statistics

\section{Introduction}

\subsection{The Hilbert--P\'olya conjecture and the dynamical viewpoint}

Since Riemann's 1859 memoir \cite{riemann1859} connected the distribution of primes to the nontrivial zeros of the $\zeta$ function, the Riemann Hypothesis --- that all nontrivial zeros $\rho = 1/2 + i\gamma_n$ lie on the critical line --- has remained central to both mathematics and mathematical physics \cite{titchmarsh1986,sarnak2000}. The Hilbert--P\'olya conjecture offers a physical route to this statement: if the ordinates $\gamma_n$ are the eigenvalues of a self-adjoint operator, their reality follows automatically \cite{schumayer2011}. This idea has motivated a long line of candidate operators and quantization schemes, from the Berry--Keating $H = xp$ proposal \cite{berry1999} and its Bender--Brody--M\"uller variant \cite{bender2017}, to the modified-commutator generalizations explored by Bishop et al.\ \cite{bishop2019}, as well as approaches based on noncommutative geometry \cite{connes1999} and Landau levels \cite{sierra2008}. We emphasize at the outset that the present work does \emph{not} claim to identify the Hilbert--P\'olya operator or to prove any statement about the zeros; it explores a specific deterministic dynamical system as a numerical model whose spectrum is compared to the zeros.

\subsection{Random matrix theory: what it does and does not assert}

A foundational result in this area is the agreement between the local statistics of the Riemann zeros and those of the GUE. Montgomery's pair-correlation conjecture \cite{montgomery1973}, together with Dyson's identification of the corresponding random-matrix form \cite{dyson1962} and Odlyzko's high-precision numerical confirmation \cite{odlyzko1987}, established this connection, which sits within the Bohigas--Giannoni--Schmit framework relating quantum chaos to RMT \cite{bohigas1984,gutzwiller1990}.

It is important to state the RMT position accurately. The GUE correspondence concerns the \emph{unfolded local} spectral statistics: one first removes the smooth, slowly varying mean density $\bar{N}(E)\sim (E/2\pi)\log(E/2\pi)$ \cite{weyl1911} --- which is dictated by the prime number theorem \cite{hadamard1896} --- so that the rescaled spacings have unit mean, and only then compares correlation functions. RMT does \emph{not} assert that a finite Wigner semicircle reproduces the global counting function of the zeros; the unfolding step removes exactly that global information. Moreover, the deviations of the zeros from pure GUE behavior are not failures of RMT but carry arithmetic content: the subleading, non-universal corrections to the pair correlation are equivalent to the Hardy--Littlewood twin-prime conjecture \cite{keating2019}.
...
In a conservative system the control parameter $a$ governs the breakup of Kolmogorov--Arnold--Moser (KAM) tori. We performed a Monte Carlo scan over $a$, sampling orbits and recording (i) the orbit survival rate (fraction of initial conditions remaining bounded) and (ii) a symbolic ``LL'' statistic that counts consecutive same-side excursions, the dynamical analogue of the consecutive-odd-cluster prohibition identified in \cite{wang2026}.
...
Statement & Status \\
\midrule
Local statistics of Riemann zeros match GUE (unfolded pair correlation) & Established \cite{montgomery1973,odlyzko1987} \\
Subleading zero correlations $\equiv$ Hardy--Littlewood twin-prime conjecture & Established \cite{keating2019} \\
Logistic band-merging symbolic dynamics $\cong$ prime sieve; $C_2$ as fixed point & Published \cite{wang2026} \\
